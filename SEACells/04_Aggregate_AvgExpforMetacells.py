...
Input:
    ...
    retainGenes.txt: target gene list (plain text, one gene per line, UTF-8)
...
def aggregate_metacells(
    ...
    gene_list_name : str
        Plain text file of target gene symbols under base_dir (one per line, UTF-8).
    out_h5ad_name : str
        Output metacell-level h5ad filename inside SEACELLS folder.
    log_name : str
        Runtime log filename inside SEACELLS folder.
    expected_genes : int
        Expected number of unique genes in retainGenes (default 20991).
    """
    ...
    # load target gene list (plain text, one gene per line, UTF-8)
    with open(fp_gene, "r", encoding="utf-8") as f:
        keep_raw = [line.strip() for line in f if line.strip()]
    ...
    assert len(keep_set) == expected_genes, \
        f"retainGenes count mismatch: {len(keep_set)} != {expected_genes}"

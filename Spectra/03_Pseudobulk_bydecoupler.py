...
subset to retainGenes before aggregation, filter groups with cell_count >= 3
only output h5ad file for downstream decoupler ULM analysis
"""
...
def main():
    ...
    gene_list_fp = os.path.join(work_dir, "retainGenes.csv")
    pb_h5ad = os.path.join(work_dir, "pseudobulk_sample_Level2_logmean.h5ad")
    min_cell_thresh = 5

    print("=" * 70)
    print("[STEP1] load retain gene list")
    gene_df = pd.read_csv(gene_list_fp, header=None)
    keep_genes = [str(g).strip() for g in gene_df[0].dropna()]
    ...
    # FIX: ensure grouping cols are str; dc.pp.pseudobulk builds index as
    ...
    # FIX: compute cell_count from original adata, keyed by obs_names string
